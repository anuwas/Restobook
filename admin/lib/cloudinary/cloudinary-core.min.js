...
! function(root, factory) {
    ...
    if ("function" == typeof define && define.amd) return define(["lodash"], factory);
    if ("object" == typeof exports) return module.exports = factory(require("lodash"));
    root.cloudinary || (root.cloudinary = {}), ref = factory(_), results = [];
    ...
    return allStrings = function(list) {
        ...
        switch (!1) {
            ...
            case !(_.isFunction("undefined" != typeof jQuery && null !== jQuery && null != (ref = jQuery.fn) ? ref.data : void 0) && _.isElement(element)):
        ...
        results = [];
        for (name in attributes) value = attributes[name], null != value ? results.push(setAttribute(element, name, value)) : results.push(removeAttribute(element, name));
        return results
    }, hasClass = function(element, name) {
        if (_.isElement(element)) return element.className.match(new RegExp("\\b" + name + "\\b"))
    }, addClass = function(element, name) {
        if (!element.className.match(new RegExp("\\b" + name + "\\b"))) return element.className = _.trim(element.className + " " + name)
        ...
        if (valueIsBorderBox = !0, val = "width" === name ? elem.offsetWidth : elem.offsetHeight, styles = getStyles(elem), isBorderBox = "border-box" === cssValue(elem, "boxSizing", !1, styles), val <= 0 || null == val) {
        ...
        return extend(ArrayParam, superClass), ArrayParam.prototype.serialize = function() {
            var array, flat, t;
            return null != this.shortName ? (array = this.value(), cloudinary.Util.isEmpty(array) ? "" : (flat = function() {
                var j, len, ref, results;
                for (ref = this.value(), results = [], j = 0, len = ref.length; j < len; j++) t = ref[j], cloudinary.Util.isFunction(t.serialize) ? results.push(t.serialize()) : results.push(t);
                return results
            }.call(this), this.shortName + "_" + flat.join(this.sep))) : ""
        ...
        return extend(TransformationParam, superClass), TransformationParam.prototype.serialize = function() {
            ...
            return cloudinary.Util.isEmpty(this.value()) ? "" : cloudinary.Util.allStrings(this.value()) ? (joined = this.value().join(this.sep), cloudinary.Util.isEmpty(joined) ? "" : this.shortName + "_" + joined) : (result = function() {
                var j, len, ref, results;
                for (ref = this.value(), results = [], j = 0, len = ref.length; j < len; j++) t = ref[j], null != t && (cloudinary.Util.isString(t) && !cloudinary.Util.isEmpty(t) ? results.push(this.shortName + "_" + t) : cloudinary.Util.isFunction(t.serialize) ? results.push(t.serialize()) : cloudinary.Util.isPlainObject(t) && !cloudinary.Util.isEmpty(t) ? results.push(new Transformation(t).serialize()) : results.push(void 0));
                return results
    ...
    }(Param), parameters = {}, parameters.Param = Param, parameters.ArrayParam = ArrayParam, parameters.RangeParam = RangeParam, parameters.RawParam = RawParam, parameters.TransformationParam = TransformationParam, parameters.LayerParam = LayerParam, Condition = function() {
        ...
        return DEFAULT_CONFIGURATION_PARAMS = {
            ...
            if (cloudinary_url = "undefined" != typeof process && null !== process && null != (ref1 = process.env) ? ref1.CLOUDINARY_URL : void 0, null != cloudinary_url && (uriRegex = /cloudinary:\/\/(?:(\w+)(?:\:(\w+))?@)?([\w\.-]+)(?:\/([^?]*))?(?:\?(.+))?/, uri = uriRegex.exec(cloudinary_url), uri && (null != uri[3] && (this.configuration.cloud_name = uri[3]), null != uri[1] && (this.configuration.api_key = uri[1]), null != uri[2] && (this.configuration.api_secret = uri[2]), null != uri[4] && (this.configuration.private_cdn = null != uri[4]), null != uri[4] && (this.configuration.secure_distribution = uri[4]), query = uri[5], null != query)))
        ...
        return TransformationBase.prototype.trans_separator = "/", TransformationBase.prototype.param_separator = ",", lastArgCallback = function(args) {
            ...
            var camelKey;
            return camelKey = Util.camelCase(key), Util.contains(Transformation.methods, camelKey) ? this[camelKey](value) : this.otherOptions[key] = value, this
            ...
            switch (resultArray = function() {
                ...
                for (results = [], j = 0, len = paramList.length; j < len; j++) t = paramList[j], results.push(null != (ref2 = this.get(t)) ? ref2.serialize() : void 0);
                return results
            }.call(this), !1) {
            ...
            return transformationList = function() {
                var j, len, results;
                for (results = [], j = 0, len = transformationList.length; j < len; j++) value = transformationList[j], (Util.isArray(value) && !Util.isEmpty(value) || !Util.isArray(value) && value) && results.push(value);
                return results
        ...
        return extend(Transformation, superClass), Transformation.new = function(args) {
            ...
            return this.param(value, "dpr", "dpr", function(_this) {
                return function(dpr) {
                    return dpr = dpr.toString(), (null != dpr ? dpr.match(/^\d+$/) : void 0) ? dpr + ".0" : dpr
                }
            }(this))
        ...
        return VERSION = "2.1.8", CF_SHARED_CDN = "d3jpl91pxevbkh.cloudfront.net", OLD_AKAMAI_SHARED_CDN = "cloudinary-a.akamaihd.net", AKAMAI_SHARED_CDN = "res.cloudinary.com", SHARED_CDN = AKAMAI_SHARED_CDN, DEFAULT_POSTER_OPTIONS = {
            format: "jpg",
            resource_type: "video"
            ...
            var options;
            if (Util.isPlainObject(resourceType) && (options = resourceType, resourceType = options.resource_type, type = options.type, urlSuffix = options.url_suffix, useRootPath = options.use_root_path, shorten = options.shorten), null == type && (type = "upload"), null != urlSuffix)
                if ("image" === resourceType && "upload" === type) resourceType = "images", type = null;
                else {
            ...
            for (responsive = null != (ref = null != (ref1 = options.responsive) ? ref1 : this.config("responsive")) && ref, elements = function() {
                ...
                if (tag = elements[j], (null != (ref5 = tag.tagName) ? ref5.match(/img/i) : void 0) && (setUrl = !0, responsive && Util.addClass(tag, responsiveClass), dataSrc = Util.getData(tag, "src-cache") || Util.getData(tag, "src"), !Util.isEmpty(dataSrc))) {
                    if (dataSrc = updateDpr.call(this, dataSrc, roundDpr), HtmlTag.isResponsive(tag, responsiveClass))
                        if (containerWidth = findContainerWidth(tag), 0 !== containerWidth) {
                            switch (!1) {
                                ...
                                case !(match = /w_auto(:(\d+))?/.exec(dataSrc)):
                                    requiredWidth = applyBreakpoints.call(this, tag, containerWidth, match[2], options), requiredWidth = maxWidth(requiredWidth, tag), dataSrc = dataSrc.replace(/w_auto[^,\/]*/g, "w_" + requiredWidth)
                            }
